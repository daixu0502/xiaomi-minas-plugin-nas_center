...
serve(){ p=${REQUEST_URI:-/index.html};p=${p%%\?*};case "$p" in */app.js)static_header 'application/javascript; charset=utf-8';cat "$DIR/app.js";;*/style.css)static_header 'text/css; charset=utf-8';cat "$DIR/style.css";;*/detail.css)static_header 'text/css; charset=utf-8';cat "$DIR/detail.css";;*)static_header 'text/html; charset=utf-8';cat "$DIR/index.html";;esac;exit; }
...
request=$(printf '%s' "$body"|jq --arg action "$action" --arg pluginUser "$user" '.+{action:$action,pluginUser:$pluginUser}');response=$(printf '%s' "$request"|sudo -n "$HELPER" 2>/dev/null)||error '无法调用权限助手';printf '%s' "$response"|jq empty>/dev/null 2>&1||error '助手返回无效数据';version=$(jq -r '.version//"1.1.2"' "$INFO" 2>/dev/null||echo 1.1.2);json_header;printf '%s' "$response"|jq --arg pluginVersion "$version" '.+{pluginVersion:$pluginVersion}'
